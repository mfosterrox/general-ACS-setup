...
get_central_cr_name() {
  ...
  for name in stackrox-central-services central; do
    if oc get central "${name}" -n "${RHACS_NAMESPACE}" &>/dev/null; then
      echo "${name}"
      return 0
    fi
  done
...
if oc get deployment central -n "${RHACS_NAMESPACE}" -o yaml | grep -q "declarative-config"; then
  ...
  if [ -n "${central_cr_name}" ]; then
    log "Using RHACS Operator to enable declarative configuration (${central_cr_name})..."
...
'
    ...
    oc set volume deployment/central -n "${RHACS_NAMESPACE}" \
  ...
  oc rollout status deployment/central -n "${RHACS_NAMESPACE}" --timeout=300s
...
if [ -n "$AUTH_PROVIDER_ID" ]; then
  ...
  if [ "$group_created" != "true" ]; then
    ...
    if [ -n "$EXISTING_GROUPS" ]; then
      ...
      error ""
      error "3. Rerun auth configuration:"
      error "   bash $SCRIPT_DIR/03-configure-rhacs-auth.sh"
      exit 1
    fi
  fi
else
