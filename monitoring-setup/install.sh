...
export RHACS_NS="${RHACS_NS:-${RHACS_NAMESPACE}}"
echo ""
...
if [ -n "$GROUPS_LIST" ]; then
  ...
  if echo "$AUTH_TEST" | grep -q '"userId"'; then
    ...
    if echo "$METRICS_TEST" | grep -q "access for this user is not authorized"; then
      ...
      error "Rerun auth configuration:"
      echo "  bash $SCRIPT_DIR/03-configure-rhacs-auth.sh"
    ...
    echo ""
    warn "If it continues to fail, rerun:"
    echo "  bash $SCRIPT_DIR/03-configure-rhacs-auth.sh"
  else
