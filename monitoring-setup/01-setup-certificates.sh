...
openssl req -x509 -new -nodes -key ca.key -sha256 -days 1825 -out ca.crt \
  -subj "/CN=Monitoring Root CA/O=RHACS Monitoring" \
...
oc delete secret sample-stackrox-prometheus-tls -n "${RHACS_NAMESPACE}" 2>/dev/null || true
oc create secret tls sample-stackrox-prometheus-tls --cert=client.crt --key=client.key -n "${RHACS_NAMESPACE}"
